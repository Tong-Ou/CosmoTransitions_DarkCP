...
def traceMinimum(f, d2f_dxdt, d2f_dx2, x0, t0, tstop, dtstart, deltaX_target,
    ...
    dXdT = np.array(dXdT)
    #if debug:
    #print ('Points traced: %s' % npoints)
    #print ('Final temperatures on trace: %s %s' % (T[-1],T[-2]))
...
def tunnelFromPhase(phases, start_phase, V, dV, Tmax, makePlot=False,
...
def findAllTransitions(phases, V, dV, makePlot=False, tunnelFromPhase_args={}):
    """
    ...
    while start_phase is not None:
        ...
        trans = tunnelFromPhase(phases, start_phase, V, dV, Tmax, makePlot,
...
######################################################
def makePlots(tobj, V_, xmin=-300, xmax=300, ymin=-400, ymax=400, n=50, clevs=500, cfrac=.8, **contourParams):
    import matplotlib.pyplot as plt
    plt.figure(figsize=(10,5))
    ax = plt.subplot(121)
    # First draw contour of potential
    ...
    #print(tobj.Phi[...,1][0])
    #print(tobj.Phi[...,1][-1])
    ax.plot(tobj.Phi[:,0], tobj.Phi[:,1],'k',lw=1.5)
    ax.set_xlabel(r'H')
    ax.set_ylabel(r'S')

    #Bubble profile
    ax = plt.subplot(122)
    ax.plot(tobj.profile1D.R, tobj.profile1D.Phi, 'r')
    ax.set_xlabel('$r$')
